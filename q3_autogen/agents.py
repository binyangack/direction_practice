...
判断数据形态（据此决定画什么图）：
- 统计 group_col 是否有重复取值。若 group_col 出现重复值（同一对象多行），或存在某数值列明显是“自变量/推进列”（列名含 temperature/temp/T/温度/time/时间/压力/pressure/step 等）→ 视为【序列数据】。
...
用 print 输出关键信息：对比数据 → 每个数值列排序后的各对象数值（降序）；序列数据 → 每个对象该属性的变化（如首尾值、极值点及其对应 x）。
...
2. 若是【随自变量变化的序列/曲线】：描述该属性随自变量的变化趋势（如 热导率随温度升高而下降/先升后降、单调性、极值点及其对应取值），指出主要变化区间、量级，以及偏离线性/符合某种规律的现象。
